...
/// The Dante device's AES67 mode and RTP settings, carried in the application's
/// AdaptationData (provisional until AES70-23 defines it).
struct GetAES67Mode: REPLCommand, REPLCurrentBlockCompletable, REPLClassSpecificCommand {
  static let name = ["get-aes67-mode", "aes67-mode"]
  static let summary = "Show the Dante device's AES67 mode and RTP settings"
  ...
  func execute(with context: Context) async throws {
    let application = context.currentObject as! DanteOcaMediaTransportApplication
    let blob = try await application.$adaptationData._getValue(application, flags: [])
    let data = try blob.decode(DanteMediaTransportApplicationAdaptationData.self)
    guard data.aes67Supported else {
      context.print("AES67 not supported")
      return
    }
    context.print("mode\t\(data.aes67Mode)\tafter reboot\t\(data.aes67RebootMode)")
    context.print(
      "rtp\t\(data.rtpMulticastAddressPrefix):\(data.rtpDestinationPort)\t\(data.rtpFramesPerPacket) frames/packet\t\(Int(data.rtpReceiveLatency * 1e6)) us latency"
    )
  }
...
struct SetAES67Mode: REPLCommand, REPLCurrentBlockCompletable, REPLClassSpecificCommand {
  static let name = ["set-aes67-mode"]
  static let summary = "Enable or disable AES67 on the Dante device (takes effect after a reboot): <true|false>"
  ...
  func execute(with context: Context) async throws {
    let application = context.currentObject as! DanteOcaMediaTransportApplication
    let blob = try await application.$adaptationData._getValue(application, flags: [])
    var data = try blob.decode(DanteMediaTransportApplicationAdaptationData.self)
    data.aes67Mode = enabled ? .enabled : .disabled
    try await application.$adaptationData._setValue(application, try data.blob)
  }
